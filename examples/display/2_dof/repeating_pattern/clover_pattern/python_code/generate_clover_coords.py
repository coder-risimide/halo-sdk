...
# --- Save as C array ---
with open("clover_coords.c", "w") as f:
    f.write("float clover_coords[][2] = {\n")
    for (x, y) in coords:
        f.write(f"    {{ {x:.2f}, {y:.2f} }},\n")
    f.write("};\n")

print(f"Saved {len(coords)} coordinates scaled & transformed for robot in clover_coords.c")
